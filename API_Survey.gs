...
/**
 * Ambil konfigurasi survey dari sheet Master_Survey_Config
 * Jika sheet tidak ada, fallback ke SURVEY_CONFIG hardcoded
 */
...
/**
 * Simpan konfigurasi survey ke sheet Master_Survey_Config
 * Admin only
 */
function saveSurveyConfig(payload) {
  try {
    ...
    return withLock(function() {
      ...
      if (!sheet) {
        sheet = ss.insertSheet('Master_Survey_Config');
        var hdrs = ['config_type', 'config_key', 'config_value', 'config_label', 'config_icon', 'config_desc', 'urutan'];
        sheet.getRange(1, 1, 1, hdrs.length).setValues([hdrs]);
        sheet.getRange(1, 1, 1, hdrs.length).setBackground('#1a237e').setFontColor('#ffffff').setFontWeight('bold');
      ...
      var colCount = hdrs.length;
      var rows = [];
      payload.teams.forEach(function(t, i) {
        rows.push(['team', t.id, t.label || t.value, t.label || t.value, t.icon || '📋', t.desc || '', String(i + 1)]);
      });
      payload.criteria.forEach(function(c, i) {
        rows.push(['criteria', c.id, c.label || c.value, c.label || c.value, c.icon || '⭐', c.desc || '', String(i + 1)]);
      });
      ...
      Logger.log('Survey config saved: ' + rows.length + ' rows');
