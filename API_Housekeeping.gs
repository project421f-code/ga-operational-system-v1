...
function saveMasterCSSchedule(payload) {
  try {
    ...
    return withLock(function() {
      var sheet = getSheet(CONFIG.SHEETS.MASTER_CS_SCHEDULE);

      if (payload._rowIndex) {
        // UPDATE
        updateRowCells(CONFIG.SHEETS.MASTER_CS_SCHEDULE, payload._rowIndex, {
          lokasi_area: payload.lokasi_area,
...
function uploadAuditPhoto(base64Data, fileName) {
  try {
    ...
    // Cari atau buat folder GA_Audit_Photos
    var folderName = 'GA_Audit_Photos';
...
/**
 * Mendapatkan daftar lokasi area (dari Master CS Schedule & Master_Lokasi)
 */
  ...
  try {
    ...
    try {
      ...
      lokasiData.forEach(function(l) {
        if (l.status === 'Aktif') {
          locations.push({
            lokasi_area: l.nama_lokasi,
            tim_penanggungjawab: l.tim_penanggungjawab
          });
        }
      });
    } catch (e) {
      // Master_Lokasi mungkin belum dibuat, abaikan
    }
...
// ─── MASTER LOKASI (Housekeeping) ────────────────────────────

/**
 * Mendapatkan semua data Master Lokasi
 * Auto-create sheet jika belum ada (dengan locking)
 */
...
/**
 * Internal: Buat sheet Master_Lokasi (panggil di dalam lock)
 */
