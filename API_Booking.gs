...
function saveAssetList(payload) {
  try {
    ...
    return withLock(function() {
      var sheet = getSheet(CONFIG.SHEETS.ASSET_LIST);
      if (payload._rowIndex) { updateRowCells(CONFIG.SHEETS.ASSET_LIST, payload._rowIndex, { kategori: payload.kategori, nama_aset: payload.nama_aset, detail_kapasitas: payload.detail_kapasitas || '', status_operasional: payload.status_operasional || 'Tersedia' }); return successResponse(null, 'Aset berhasil diperbarui.'); }
      else { sheet.appendRow([payload.kategori, payload.nama_aset, payload.detail_kapasitas || '', payload.status_operasional || 'Tersedia']); return successResponse(null, 'Aset berhasil ditambahkan.'); }
    });
  } catch (e) { return errorResponse(e.message); }
}
